...
umwege_mean %>% 
  ...
  ylab("Bereitschaft für Umwege [min]") +
  xlab("") +
  theme_grey(base_size = 12)
```
...
```{r, echo = F, message = F, fig.width= 8, fig.height=11}
umwege_mean_typ %>% 
...
for(umweg in sorted_umwege_order) {
  plot(bar_chart(filter(umwege_mean_typ, name == umweg), Typ_all, value,
         ...
         xlab("") +
         ylab("durchschnittliche Bereitschaft für Umwege [min]") +
...
```{r, echo = F, message = F, fig.width = 9, fig.height= 6}
